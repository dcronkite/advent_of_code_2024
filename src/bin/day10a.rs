...
fn find_trails(map: &Vec<Vec<usize>>, x: usize, y: usize, elevation: usize) -> HashSet<(usize, usize)> {
    let mut trails: HashSet<(usize, usize)> = HashSet::new();
    if elevation == 9 {
        trails.insert((x, y));
        return trails;
    }
